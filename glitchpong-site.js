...
function injectGameById(gameId, options = {}) {
  ...
  function startPong(options = {}) {
    ...
    const rightX = canvas.width - leftX - paddleW;
      ...
      if (isMenuDemo) {
        ...
        switch (aiDifficulty) {
          case "easy":
            ...
            break;
          case "hard":
            smoothing = 0.085 + getHeatRatio() * 0.02; // current + heat boost
            errorRange = 0;
            canDash = true;
            dashThresholdMult = 1;
            dashWindowMult = 1;
            ...
            break;
          default: // "medium"
            smoothing = 0.055 + getHeatRatio() * 0.01; // noticeably slower than hard
            errorRange = 22;             // small imprecision
            canDash = true;
            dashThresholdMult = 1.5;     // needs bigger gap before dashing
            dashWindowMult = 0.6;        // shorter dash activation window
            retreatBias = 0.35;
        }
      }
    ...
    function drawBossFight() {
      ...
      if (bossData.projectile) {
        ctx.beginPath();
        ctx.arc(bossData.projectile.x, bossData.projectile.y, bossData.projectile.r, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(255,255,255,0.96)";
        ctx.shadowColor = "rgba(255,64,64,0.86)";
        ctx.shadowBlur = 18;
        ctx.fill();
        ctx.shadowBlur = 0;
      }
    ...
    function drawArenaBase(alpha = 0.18, showPaddles = true) {
      ...
      if (leftDashCooldown > 0) {
        ...
        ctx.beginPath();
        ctx.arc(ballX, ballY, BALL_R + heatRatio * 1.2, 0, Math.PI * 2);
    ...
    function loop() {
      ...
      if (!roundFrozen) {
        ...
        if (ballX <= leftX + paddleW && ballY >= leftY - leftShieldPad && ballY <= leftY + paddleH + leftShieldPad) {
          ...
          heat = Math.min(HEAT_MAX, heat + HEAT_PER_HIT);
          rallyHits += 1;
          combo += 1;
          comboTimer = COMBO_MAX_TIME;
          ballOwner = "blue";
          ...
          if (!bossModeActive && !bossTriggeredThisRally && heat >= HEAT_MAX) startBossFight();
    ...
    addListener(document, "keydown", (e) => {
      ...
      if ((e.key === "q" || e.key === "Q") && currentScreen === "game") {
        e.preventDefault();
        heat = HEAT_MAX;
        if (!bossModeActive && !bossTriggeredThisRally) {
          startBossFight();
        }
        console.log("[DEBUG] Heat forced to 100%. Boss triggered.");
        return;
      }
      if (e.key === "Escape") {
        e.preventDefault();
